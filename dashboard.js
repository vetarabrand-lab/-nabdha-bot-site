const SUPABASE_URL = 'https://anptuwcfvfcjqtqqnirt.supabase.co';
  ...
  async function loadDashboard(){
    ...
    const { data: clients, error } = await supabaseClient
      .from('clients')
      .select('id, business_name, business_name_ar, owner_email, owner_phone, plan_id, subscription_status, trial_ends_at, is_paid, user_id, created_at, extra_message_credits, whatsapp_connection_status, whatsapp_connection_checked_at, whatsapp_connection_error, cart_coupon_addon_enabled, current_period_ends_at')
  ...
  function renderDaysRemaining(c){
    ...
    if(c.subscription_status === 'trial' && c.trial_ends_at){
      ...
      if(daysLeft <= 0){
        return '<span class="badge expired">تنتهي اليوم</span>';
      }
      return '<span class="badge trialdays">' + daysLeft + ' يوم متبقي (تجربة)</span>';
    }
    if(c.subscription_status === 'active' && c.is_paid){
      if(c.current_period_ends_at){
        const msLeft = new Date(c.current_period_ends_at).getTime() - Date.now();
        const daysLeft = Math.ceil(msLeft / (1000 * 60 * 60 * 24));
        if(daysLeft <= 0){
          return '<span class="badge expired">انتهت المدة — يحتاج تجديد</span>';
        }
        if(daysLeft <= 5){
          return '<span class="badge trialdays">' + daysLeft + ' يوم متبقي — قرب التجديد</span>';
        }
        return '<span class="badge permanent">' + daysLeft + ' يوم متبقي</span>';
      }
